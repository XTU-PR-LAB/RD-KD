...
def upflow4(flow, mode='bilinear'):
    ...
    with torch.autocast(device_type='cuda', dtype=torch.loat16, enabled=False):
...
def sample_points(init_normal, gt_norm_mask, sampling_ratio, beta):
    ...
    uncertainty_map = -1 * init_normal[:, -1, :, :]
...
class RAFTDepthNormalDPT5(nn.Module):
    ...
    def forward(self, vit_features, **kwargs):
        ...
        encoder_features = self.token2feature(vit_features)
        ...
        ref_feat= self.decoder_mono(encoder_features)
        ...
        depth_pred, binmap = self.regress_depth(feature_map)
        normal_pred = self.pred_normal(feature_map, normal_confidence_map)
